...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  { id: 11, date: '2026-05-13', text: '🔥 Стрики, поздравления и заморозки. Теперь у каждой привычки свой стрик (дни подряд), а ещё общий стрик активности — за дни когда ты закрыл хотя бы одну привычку. На рубежах (3, 5, 7, 10, 14, 15, 30, 60, 100 дней) бот пришлёт поздравление с салютом или огоньком. Если пропустил день — заморозка стрика спасёт его автоматически. Заморозки копятся за каждые 5 дней общего стрика, максимум 2 в инвентаре. Видны в /stats и в weekly view (значок 🧊). Тексты напоминаний теперь подстраиваются под ситуацию — после пропуска бот напомнит мягче и теплее' },
];
