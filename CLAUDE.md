...
- **Weekly calendar**: 7-дневная сетка с эмодзи (🟢 сделано / 🔴 пропущено / 🧊 заморожено через freeze / ⏸️ выходной / ⚪ ещё не наступило), листание по неделям
...
🔴 **Полный запрет на имена в комментариях.** Никаких `(<name> <date>)`, `// per user request`, `// автор поправил`. Полный бан на любые упоминания людей в коде.

🔴 **Комментарии — ТОЛЬКО про код, не про историю.** Не пишем:
- историю багов («раньше тут было X, потом сломалось, потом починили»)
- даты обнаружения проблем
- ссылки на review/обсуждения
- примеры того что когда-то было неправильно

Комментарий нужен ТОЛЬКО когда:
- WHY непонятен из кода (скрытая инвариантность, неочевидное ограничение API, workaround для конкретного бага в библиотеке)
- WHY помог бы будущему читателю не наступить на грабли

Если убрать комментарий и читатель ничего не потеряет — комментарий лишний. Контекст изменений идёт в PR description / commit message, не в код.
