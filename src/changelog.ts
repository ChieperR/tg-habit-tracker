...
export const CHANGELOG: ChangelogEntry[] = [
  ...
  { id: 10, date: '2026-05-07', text: '💬 Появилась команда /feedback — расскажи что в боте не работает, чего не хватает или поделись идеей. Автор бота получит твоё сообщение и даст ответ :)' },
  { id: 11, date: '2026-05-14', text: '🔥 Стрики сделаны более явными, в разных напоминаниях добавлена вариативность текстов. Добавлены заморозки за стрики, чтобы не так страшно было разочек пропустить привычку)' },
  { id: 12, date: '2026-05-15', text: '📊 Пофикшена сетка активности в /stats: текущая неделя теперь всегда в правой колонке, видны все недавние отметки. До этого последние дни могли выходить за пределы графика и казалось, что отметок нет' },
  { id: 13, date: '2026-05-15', text: '🔥 Пофикшен расчёт стриков в /stats: пропуск дня (без freeze) теперь действительно обрывает стрик. До этого из-за бага некоторые пропущенные дни считались как «продолжение» стрика' },
  { id: 14, date: '2026-05-29', text: '🌙 Отметка привычки в вечернем напоминании теперь засчитывается за день когда оно пришло: можно быстро отмечать привычки и после полуночи!' },
  { id: 15, date: '2026-06-01', text: '✏️ Привычку теперь можно переименовать: открой детали привычки и нажми «Переименовать» над кнопкой удаления' },
];
